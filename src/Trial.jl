...
1. Calls to `play` to present an objects generated by `sound` before the moment.
2. Calls to `display` to present an object generated by `render` before the moment.
3. Calls to `record` to save something to the data file (usually after any calls to
`play` or `display`)
4. Simple programming logic (e.g. `if` and `elseif` and `else`).

Note that Julia compiles functions on demand (known as JIT compilation), which
can be slow the first time a function runs.  To avoid JIT compilation, any
functions called direclty by a moment is first precompiled. Futher, many of the
methods implemented in Psychotask have also been repcompiled.
"""
...
- presenting graphical objects that have been pre-rendered (using `render`)
